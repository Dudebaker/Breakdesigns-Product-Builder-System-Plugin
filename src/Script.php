<?php
	...
	class PlgSystemBreakdesignsproductbuilderInstallerScript
	{
		public const MAX_VERSION_JOOMLA = '6.0.0';
		public const MIN_VERSION_JOOMLA = '5.2.0';
		public const MIN_VERSION_PHP = '8.2.0';
		
		protected string $extensionName = 'Breakdesigns Product Builder System Plugin';
		
		protected string $elementName = 'breakdesignsproductbuilder';
		...
		private function checkVersionJoomla() : bool
		{
			...
			{
				Factory::getApplication()?->enqueueMessage(Text::sprintf('PLG_SYSTEM_BREAKDESIGNS_PRODUCT_BUILDER_ERROR_JOOMLA_VERSION', $this->extensionName, self::MIN_VERSION_JOOMLA), 'error');
			...
			{
				Factory::getApplication()?->enqueueMessage(Text::sprintf('PLG_SYSTEM_BREAKDESIGNS_PRODUCT_BUILDER_ERROR_JOOMLA_VERSION_MAX', $this->extensionName, self::MAX_VERSION_JOOMLA), 'error');
		...
		private function checkVersionPhp() : bool
		{
			...
			{
				Factory::getApplication()?->enqueueMessage(Text::sprintf('PLG_SYSTEM_BREAKDESIGNS_PRODUCT_BUILDER_ERROR_PHP_VERSION', $this->extensionName, self::MIN_VERSION_PHP), 'error');
		...
		private function enableExtension() : void
		{
			...
			{
				Factory::getApplication()?->enqueueMessage(Text::_('PLG_SYSTEM_BREAKDESIGNS_PRODUCT_BUILDER_ERROR_NOT_FOUND'), 'error');
				return;
			}
			...
			Factory::getApplication()?->enqueueMessage(Text::_('PLG_SYSTEM_BREAKDESIGNS_PRODUCT_BUILDER_ERROR_FAILED_TO_ACTIVATE'), 'error');
		}
		#endregion
	}
